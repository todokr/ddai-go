...
func TestBuffer(t *testing.T) {
	...
	for i := range 5 {
		...
		buff1, err := bm.Pin(file.NewBlockID("testfile", 0))
		if err != nil {
		...
		/* buff3 */ _, err = bm.Pin(file.NewBlockID("testfile", 2))
		if err != nil {
			t.Fatalf("bm.Pin(2): %v", err)
		}
		/* buff4 */ _, err = bm.Pin(file.NewBlockID("testfile", 3))
		if err != nil {
...
func TestBufferManager(t *testing.T) {
	...
	for i, b := range buff {
		if b != nil {
			if wants[i] < 0 {
				t.Errorf("buff[%d] pinned to block %v, but should be unpinned", i, b.Block.Index)
			}
			if b.Block.Index != wants[i] {
				t.Errorf("buff[%d] pinned to block %v, but should be block %d", i, b.Block.Index, wants[i])
			}
		} else {
			if wants[i] >= 0 {
